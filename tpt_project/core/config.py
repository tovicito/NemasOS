...
class Configuracion:
    ...
    def __init__(self):
        ...
        self.DIR_GIT_CLONES = self.DIR_CACHE / "clones" # Para el fallback de Git
        ...
        self.ARCHIVO_SETTINGS = self.DIR_CONFIG / "settings.json"
    ...
    def load_settings(self) -> dict:
        """Carga la configuración desde settings.json o devuelve los valores por defecto."""
        if not self.ARCHIVO_SETTINGS.exists():
            return self.SETTINGS_POR_DEFECTO
        try:
            with open(self.ARCHIVO_SETTINGS, "r", encoding="utf-8") as f:
                return json.load(f)
        except (json.JSONDecodeError, IOError):
            return self.SETTINGS_POR_DEFECTO # Devolver valores por defecto en caso de error

    def save_settings(self, settings: dict):
        """Guarda la configuración en settings.json."""
        self.asegurar_directorios()
        with open(self.ARCHIVO_SETTINGS, "w", encoding="utf-8") as f:
