...
class CLI:
    ...
    def _create_parser(self):
        ...
        parser = argparse.ArgumentParser(
            prog="tpt",
            description=Panel(_("TPT - La Herramienta de Paquetes Total"), title="[bold green]TPT v5.0[/bold green]", expand=False)
        )
    ...
    def _handle_gui(self, args):
        ...
        try:
            ...
            self.logger.critical(_("No se pudo lanzar la GUI: {}").format(e))
            self.logger.warning(_("Asegúrate de que tienes las dependencias de GTK4 instaladas."))
            self.logger.warning(_("En Debian/Ubuntu: sudo apt install python3-gi python3-gi-cairo gir1.2-gtk-4.0 gir1.2-adw-1"))
            self.logger.warning(_("O con pip: pip install PyGObject"))
            sys.exit(1)
